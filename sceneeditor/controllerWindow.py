...
class controllerWindow(AppShell):
    ...
    def createInterface(self):
        ...
        Label(Interior, text='   ').pack(side=tkinter.LEFT, expand=False)
        widget = self.createcomponent(
    ...
    def ok_press(self):
        ...
        if self.controllType == 'Keyboard':
            ...
            messenger.send('ControlW_controlSetting',
                           ['Keyboard', [self.nodePath, self.keyboardMapDict, self.keyboardSpeedDict]])
        self.quit()
        return
    ...
    def enableControl(self):
        ...
        if self.controllType == 'Keyboard':
            ...
            for index in self.keyboardSpeedDict:
                self.keyboardSpeedDict[index] = float(self.widgetsDict['Keyboard-' + index].getvalue())
            messenger.send('ControlW_controlEnable',
                           ['Keyboard', [self.nodePath, self.keyboardMapDict, self.keyboardSpeedDict]])
        return
    ...
    def saveKeepControl(self):
        ...
        if self.controllType == 'Keyboard':
            ...
            for index in self.keyboardSpeedDict:
                self.keyboardSpeedDict[index] = float(self.widgetsDict['Keyboard-' + index].getvalue())
            print(self.nodePath)
            messenger.send('ControlW_saveSetting',
                           ['Keyboard', [self.nodePath, self.keyboardMapDict, self.keyboardSpeedDict]])
        return
