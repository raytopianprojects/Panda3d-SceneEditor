...
class dataHolder:
    ...
    keyboardMapDict = {'KeyForward': 'arrow_up',
                       ...
                       'KeyScaleZUp': '',
                       'KeyScaleZDown': ''}
    keyboardSpeedDict = {'SpeedForward': 10,
                         'SpeedBackward': 10,
                         'SpeedLeft': 10,
                         'SpeedRight': 10,
                         'SpeedUp': 10,
    ...
    def getAllObjNameAsList(self):
        ...
        list = list + l(self.ModelDic.keys()) \
               + l(self.ActorDic.keys()) + l(self.collisionDict.keys()) \
               + l(self.dummyDict.keys()) + l(self.particleNodes.keys()) \
               + l(self.lightManager.getLightList())
        return list
    ...
    def loadScene(self):
        ...
        self.Scene = self.theScene.SavedScene(0, seParticleEffect, seParticles,
        ...
        for light in self.Scene.LightDict:
            ...
            #print type
            if type == 'ambient':
                self.lightManager.create('ambient', alight.getColor(), name=alight.getName(),
                                         tag=thenode.getTag("Metadata"))
            elif type == 'directional':
                #print alight.getPoint()
                #print alight.getDirection()
                self.lightManager.create('directional', alight.getColor(), alight.getSpecularColor(), thenode.getPos(),
                                         thenode.getHpr(), name=alight.getName(), tag=thenode.getTag("Metadata"))
            elif type == 'point':
                atten = alight.getAttenuation()
                #print alight.getPoint()
                self.lightManager.create('point', alight.getColor(), alight.getSpecularColor(), thenode.getPos(),
                                         Vec3(1, 0, 0), atten.getX(), atten.getY(), atten.getZ(), name=alight.getName(),
                                         tag=thenode.getTag("Metadata"))
            elif type == 'spot':
                atten = alight.getAttenuation()
                self.lightManager.create('spot', alight.getColor(), alight.getSpecularColor(), thenode.getPos(),
                                         thenode.getHpr(), atten.getX(), atten.getY(), atten.getZ(),
                                         alight.getExponent(), name=alight.getName(), tag=thenode.getTag("Metadata"))
            else:
        ...
        for effect in self.Scene.particleDict:
            ...
            emitter.setPosHprScale(theeffect.getX(), theeffect.getY(), theeffect.getZ(), theeffect.getH(),
                                   theeffect.getP(), theeffect.getR(), theeffect.getSx(), theeffect.getSy(),
                                   theeffect.getSz())
